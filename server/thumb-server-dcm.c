...
#include "thumb-server-dcm-debug.h"
...
#define THUMB_SERVER_DCM_DCM_SVC_PACKAGE "com.samsung.dcm-svc"
#define THUMB_SERVER_DCM_PARAM_SCAN_OPERATION "SCAN_OPERATION"
#define THUMB_SERVER_DCM_PARAM_SCAN_ALL "DCM_SCAN_ALL"
#define THUMB_SERVER_DCM_PARAM_SCAN_SINGLE "DCM_SCAN_SINGLE"
#define THUMB_SERVER_DCM_PARAM_STOP_ALL_SCAN "STOP_ALL_SCAN"
#define DCM_SVC_PARAM_FILE_PATH "FILE_PATH"

#define DCM_SVC_NAME "dcm-svc"
#define DCM_SVC_EXEC_NAME "/usr/bin/dcm-server"
...
gboolean __thumb_server_dcm_agent_timer()
{
	if (g_dcm_server_extracting) {
		dcm_dbg("Timer is called.. But dcm-svc[%d] is busy.. so timer is recreated", g_dcm_server_pid);
	...
	g_dcm_timer_id = 0;
	dcm_dbg("Timer is called.. Now killing dcm-svc[%d]", g_dcm_server_pid);
	...
	if (g_dcm_server_pid > 0) {
		...
		if (!_thumb_server_dcm_send_msg(THUMB_SERVER_DCM_MSG_STOP, 0, NULL, THUMB_SERVER_DCM_PORT_DCM_SVC_APP_RECV)) {
			dcm_dbgE("_thumb_server_dcm_send_msg is failed");
			g_shutdowning_dcm_server = FALSE;
		}
		usleep(200000);
		g_dcm_communicate_sock = 0;
	} else {
		dcm_dbgE("g_server_pid is %d. Maybe there's problem in thumbnail-server", g_dcm_server_pid);
	}
...
int __thumb_strcopy(char *res, const int size, const char *pattern, const char *str1)
{
	...
	if (!res || !pattern || !str1) {
		dcm_dbgE("parameta is invalid");
		return MS_MEDIA_ERR_INVALID_PARAMETER;
	}

	if (real_size < strlen(str1)) {
		dcm_dbgE("size is wrong");
		return MS_MEDIA_ERR_INVALID_PARAMETER;
	}

	len = snprintf(res, real_size, pattern, str1);
	if (len < 0) {
		dcm_dbgE("snprintf failed");
		return MS_MEDIA_ERR_INVALID_PARAMETER;
	}
	...
	while (!readdir_r(pdir, &pinfo, &result)) {
		...
		if (fp) {
			if (fgets(buff, 128, fp) == NULL)
				dcm_dbgE("fgets failed");
			fclose(fp);

			if (strstr(buff, DCM_SVC_NAME)) {
				dcm_dbgE("%s proc is already running", buff);
				ret = TRUE;
				break;
			}
		} else {
			dcm_dbgE("Can't read file [%s]", path);
		}
	}
...
/*gboolean __thumb_server_dcm_agent_recv_msg_from_server()
{
	...
	if (g_dcm_communicate_sock <= 0) {
		if (__thumb_server_dcm_create_socket(&g_dcm_communicate_sock, THUMB_SERVER_DCM_PORT_DCM_SVC_APP_RECV) < 0) {
			dcm_dbgE("ms_ipc_create_server_socket failed");
			return FALSE;
		}
	}
	...
	if ((recv_msg_size = read(g_dcm_communicate_sock, &recv_msg, sizeof(thumb_server_dcm_ipc_msg_s))) < 0) {
		if (errno == EWOULDBLOCK) {
			dcm_dbgE("Timeout. Can't try any more");
			return MEDIA_THUMB_ERROR_NETWORK;
		} else {
			dcm_dbgE("recv failed : %s", strerror(errno));
			return MEDIA_THUMB_ERROR_NETWORK;
		}
	}
	...
	if (!(recv_msg.msg_type >= 0 && recv_msg.msg_type < THUMB_SERVER_DCM_MSG_MAX)) {
		dcm_dbgE("IPC message is wrong!");
		return  MEDIA_THUMB_ERROR_NETWORK;
	}

	if (recv_msg.msg_type == THUMB_SERVER_DCM_MSG_SERVER_READY) {
		dcm_dbg("DcmSvc server is ready");
...
gboolean __thumb_server_dcm_agent_execute_server()
{
	...
	if (pid < 0) {
		...
		dcm_dbg("Child process is %d", pid);
...
/*	if (!__thumb_server_dcm_agent_recv_msg_from_server()) {
		dcm_dbgE("_thumb_server_dcm_agent_recv_msg_from_server is failed");
		return FALSE;
	}*/

	dcm_dbg("Exec DCM_SVC: end");
...
static int __thumb_server_dcm_process_recv_msg(thumb_server_dcm_ipc_msg_s *recv_msg)
{
	...
	if (g_shutdowning_dcm_server) {
		dcm_dbgE("Thumb server is shutting down... wait for complete");
		usleep(10000);
	...
	if (g_folk_dcm_server == FALSE && g_dcm_server_extracting == FALSE) {
		if(__thumb_server_dcm_check_process() == FALSE) {  // dcm-svc is running?
			dcm_dbgW("Dcm-svc server is not running.. so start it");
			memcpy(&last_msg, recv_msg, sizeof(thumb_server_dcm_ipc_msg_s));
			if (!__thumb_server_dcm_agent_execute_server()) {
				dcm_dbgE("__thumb_server_dcm_agent_execute_server is failed");
				return MS_MEDIA_ERR_SOCKET_INTERNAL;
			} else {
				__thumb_server_dcm_create_timer(g_dcm_timer_id);
				return err;
	...
	if (recv_msg->msg_type == THUMB_SERVER_DCM_MSG_SCAN_ALL) {
		...
		if (recv_msg->msg_size > 0) {
			...
			if (file_path == NULL) {
				dcm_dbgE("Failed to get file path!");
				return MS_MEDIA_ERR_INVALID_PARAMETER;
			}

			dcm_dbgW("Send socket message to dcm-svc");
			...
			dcm_dbgE("Invalid receive message!");
			err = MS_MEDIA_ERR_SOCKET_INTERNAL;
		}
	} else if (recv_msg->msg_type == THUMB_SERVER_DCM_MSG_SERVER_READY) {
		dcm_dbgW("Recieve ready message from dcm-svc, send message to dcm-svc again");
...
static int __thumb_server_dcm_accept_tcp_socket(int serv_sock, int* client_sock)
{
	...
	if ((sockfd = accept(serv_sock, (struct sockaddr*)&client_addr, &client_addr_len)) < 0) {
		dcm_dbgE("accept failed : %s", strerror(errno));
		*client_sock  = -1;
...
static gboolean __thumb_server_dcm_read_dcm_recv_socket(GIOChannel *src, GIOCondition condition, gpointer data)
{
	...
	if (err != MS_MEDIA_ERR_NONE) {
		dcm_dbgE("Failed to accept tcp socket! err: %d", err);
		return TRUE;
	}
	...
	if (err != MS_MEDIA_ERR_NONE) {
		dcm_dbgE("Failed to receive tcp msg! err: %d", err);
	...
	if (err != MS_MEDIA_ERR_NONE) {
		dcm_dbgE("Error ocurred when process received message: %d", err);
		goto DCM_IPC_READ_TCP_SOCKET_FAILED;
	}
...
DCM_IPC_READ_TCP_SOCKET_FAILED:
	...
	if (close(client_sock) < 0) {
		dcm_dbgE("close failed [%s]", strerror(errno));
	}
...
static int __thumb_server_dcm_create_socket(int *socket_fd, thumb_server_dcm_port_type_e port)
{
	...
	if ((sock = socket(PF_FILE, SOCK_STREAM, 0)) < 0) {
		dcm_dbgE("socket failed: %s", strerror(errno));
	...
	if (bind_success == false) {
		dcm_dbgE("bind failed : %s %d_", strerror(errno), errno);
		close(sock);
		return MS_MEDIA_ERR_SOCKET_BIND;
	}
	dcm_dbg("bind success");

	/* Listenint */
	if (listen(sock, SOMAXCONN) < 0) {
		dcm_dbgE("listen failed : %s", strerror(errno));
		close(sock);
		return MS_MEDIA_ERR_SOCKET_CONN;
	}
	dcm_dbg("Listening...");
	...
	if (chmod(DCM_IPC_PATH[port], 0660) < 0)
		dcm_dbgE("chmod failed [%s]", strerror(errno));
...
gboolean _thumb_server_dcm_thread(void *data)
{
	...
	dcm_dbgE( "[GD] _thumb_server_dcm_thread!S");
	...
	if (err != MS_MEDIA_ERR_NONE) {
		dcm_dbgE("Failed to create socket! err: %d", err);
		return FALSE;
	}
	...
	dcm_dbg("********************************************");
	dcm_dbg("*** Thumb Server DCM thread is running   ***");
	dcm_dbg("********************************************");

	/* Start to run main event loop for dcm thread */
	g_main_loop_run(g_dcm_thread_mainloop);

	dcm_dbg("*** Thumb Server DCM thread will be closed ***");
...
int _thumb_server_dcm_send_msg(thumb_server_dcm_ipc_msg_type_e msg_type, uid_t uid, const char *msg, thumb_server_dcm_port_type_e port)
{
	if (port < 0 || port >= THUMB_SERVER_DCM_PORT_MAX) {
		dcm_dbgE("Invalid port! Stop sending message...");
		return MS_MEDIA_ERR_SOCKET_SEND;
	}

	dcm_dbgE( "[GD] _thumb_server_dcm_send_msg to %d ", port);
	...
	if (connect(socket_fd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) {
		dcm_dbgE("connect error : %s", strerror(errno));
		close(socket_fd);
		return MS_MEDIA_ERR_SOCKET_CONN;
	}
	...
	if (send(socket_fd, &send_msg, sizeof(send_msg), 0) != sizeof(send_msg)) {
		dcm_dbgE("send failed : %s", strerror(errno));
		close(socket_fd);
		return MS_MEDIA_ERR_SOCKET_SEND;
	}
...
void _thumb_server_dcm_quit_main_loop()
{
	if (g_dcm_thread_mainloop != NULL) {
		dcm_dbgW("Quit DCM thread main loop!");
		g_main_loop_quit(g_dcm_thread_mainloop);
	} else {
		dcm_dbgE("Invalid DCM thread main loop!");
	}
}
...
void thumb_server_dcm_reset_server_status()
{
	...
	if (g_dcm_server_extracting) {
		...
		dcm_dbgE("Thumbnail server is dead when processing all-thumbs extraction");
